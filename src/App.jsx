...
const gallery = [
  { src: "/img/gal-lambo.jpg", label: "Lamborghini · Ceramic" },
  { src: "/img/gal-ferrari.jpg", label: "Ferrari · Detailing" },
  { src: "/img/gal-audi.jpg", label: "Audi · Paint Correction" },
  { src: "/img/gal-jaguar.jpg", label: "Jaguar · Full Detail" },
  { src: "/img/gal-subaru.jpg", label: "Subaru · Coating" },
  { src: "/img/gal-mercedes.jpg", label: "Mercedes · Paint Correction" },
  { src: "/img/gal-interior.jpg", label: "Interior · Deep Clean" },
  { src: "/img/gal-seat.jpg", label: "Seats · Restored" },
];

const beforeAfters = [
  { label: "Paint Correction", before: "/img/gal-audi.jpg", after: "/img/svc-ppf.jpg" },
  { label: "Full Detail", before: "/img/gal-jaguar.jpg", after: "/img/svc-ceramic.jpg" },
  { label: "Interior", before: "/img/gal-seat.jpg", after: "/img/gal-interior.jpg" },
];
  ...
  return (
    <Reveal>
      <div style={{ textAlign: "center", marginBottom: 36 }}>
        ...
        <h2 className="display" style={{ fontSize: "clamp(28px, 7vw, 46px)", fontWeight: 700 }}>{title}</h2>
...
function HomePage({ go }) {
  ...
  return (
    <>
      {/* HERO with animated red car on a stage */}
      <header className="hero">
        ...
        <div className="wrap hero-inner">
          <div className="hero-copy">
            ...
            <p className="eyebrow" style={{ marginBottom: 14 }}>Detailing Beyond Perfection</p>
            <h1 className="display hero-h1">
              <span className="line1 shine-text">Your Car.</span>
              <span className="line2 shine-text">Showroom Shine.</span>
              <span className="line3 shine-text">At Your Door.</span>
            </h1>
            <p className="hero-sub">
              Premium ceramic coating, graphene coating & PPF — installed at your home by master technicians. <strong style={{ color: "#fff" }}>We come to you.</strong>
            </p>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              <a href={waLink("Hi ShineX! I want a free quote.")} target="_blank" rel="noreferrer" className="btn btn-red">Get Free Quote</a>
              <button onClick={() => scrollTo("services")} className="btn btn-ghost">View Services</button>
            </div>
          </div>
          <div className="hero-car-wrap">
            <div className="stage" />
            <img src="/img/hero-car.png" alt="Premium car detailing" className="hero-car" />
          </div>
        </div>
    ...
    <div className={`sx-root ${offerOpen ? "has-offer" : ""}`}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
        ...
        /* HERO */
        .hero { position: relative; min-height: 100svh; padding-top: 92px; display: flex; flex-direction: column; justify-content: center; overflow: hidden; }
        .hero-glow { position: absolute; top: 30%; left: 50%; width: 120vw; height: 70vh; transform: translateX(-50%); background: radial-gradient(ellipse at center, rgba(225,6,0,0.22) 0%, transparent 60%); filter: blur(20px); }
        .hero-grid { position: absolute; inset: 0; background-image: linear-gradient(rgba(255,255,255,0.025) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.025) 1px, transparent 1px); background-size: 44px 44px; mask-image: radial-gradient(ellipse at 50% 40%, #000 30%, transparent 75%); }
        .hero-inner { position: relative; z-index: 2; display: flex; flex-direction: column; gap: 10px; padding: 0 20px; }
        ...
        .hero-h1 { font-size: clamp(40px, 11vw, 76px); font-weight: 700; line-height: 1.02; margin-bottom: 16px; }
        .hero-h1 span { display: block; opacity: 0; transform: translateY(20px); animation: rise .7s ease forwards; }
        .hero-h1 .line1 { animation-delay: .15s, 0s; } .hero-h1 .line2 { animation-delay: .35s, .4s; } .hero-h1 .line3 { animation-delay: .55s, .8s; }
        @keyframes rise { to { opacity: 1; transform: none; } }
        .hero-sub { font-size: clamp(15px, 4vw, 18px); color: #b9b9c0; max-width: 520px; margin: 0 auto 26px; line-height: 1.6; opacity: 0; animation: rise .7s ease .7s forwards; }
        .hero-copy > div:last-child { opacity: 0; animation: rise .7s ease .85s forwards; justify-content: center; }
        .hero-car-wrap { position: relative; margin-top: 24px; display: flex; justify-content: center; }
        .hero-car { width: 100%; max-width: 560px; height: auto; filter: drop-shadow(0 30px 40px rgba(0,0,0,0.7)); animation: floatcar 5s ease-in-out infinite, carin 1s ease .4s both; }
        @keyframes carin { from { opacity: 0; transform: translateX(40px) scale(.96); } to { opacity: 1; transform: none; } }
        @keyframes floatcar { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-12px); } }
        .stage { position: absolute; bottom: 6%; left: 50%; transform: translateX(-50%); width: 78%; height: 40px; background: radial-gradient(ellipse at center, rgba(225,6,0,0.5) 0%, rgba(225,6,0,0) 70%); filter: blur(10px); }
        @media (prefers-reduced-motion: reduce) { .hero-car { animation: none; } .hero-h1 span, .hero-sub, .hero-copy > div:last-child { opacity: 1; animation: none; } }
        @media (min-width: 860px) {
          .hero-inner { flex-direction: row; align-items: center; text-align: left; }
          .hero-copy { flex: 1; text-align: left; }
          .hero-copy > div:last-child { justify-content: flex-start; }
          .hero-car-wrap { flex: 1.1; margin-top: 0; }
          .hero-car { max-width: 620px; }
        }
        .hero-stats { position: relative; z-index: 2; display: flex; gap: 28px; justify-content: center; margin-top: 28px; padding: 24px 20px 0; border-top: 1px solid rgba(255,255,255,0.08); flex-wrap: wrap; }
